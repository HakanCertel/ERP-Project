...
{
    public class HizmetBilgileriBll : BaseHareketBll<HizmetBilgileri, SenfoniErpContext>, IBaseHareketSelectBll<HizmetBilgileri>
    {
        public IEnumerable<BaseHareketEntity> List(Expression<Func<HizmetBilgileri, bool>> filter)
        {
            return List(filter, x => new HizmetBilgileriL
            {
                ...
                GittigiOkulAgi=x.GittigiOkul.OkulAdi,                
        ...
        public IEnumerable<FaturaAlinanHizmetlerL> FaturaPlaniList(Expression<Func<HizmetBilgileri, bool>> filter)
        {
            return List(filter,x=>new
            {
                ...
                HizmetBilgileri = x.Tahakkuk.HizmetBilgileri.Where(y => y.HizmetId == x.HizmetId).GroupBy(y=>y.HizmetId).DefaultIfEmpty()
                .Select(y => new
                {
                ...
                IndirimBilgileri = x.Tahakkuk.IndirimBilgileri.Where(y => y.HizmetId == x.HizmetId).GroupBy(y => y.HizmetId).DefaultIfEmpty()
                .Select(y => new
                {
